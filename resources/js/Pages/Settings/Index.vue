...
const form = useForm({
    ...
    spr_signatures: {
        city: props.settings.spr_signatures?.city || 'Jakarta Selatan',
        sig1_title: props.settings.spr_signatures?.sig1_title || 'Sales Manager',
        sig1_name: props.settings.spr_signatures?.sig1_name || 'Dhany Nur',
        sig1_image: props.settings.spr_signatures?.sig1_image || null,
        sig2_title: props.settings.spr_signatures?.sig2_title || 'Direktur',
        sig2_name: props.settings.spr_signatures?.sig2_name || 'Luhur Wira Pramudya',
        sig2_image: props.settings.spr_signatures?.sig2_image || null,
        sig3_title: props.settings.spr_signatures?.sig3_title || 'Pemesan Utama',
        sig3_name: props.settings.spr_signatures?.sig3_name || '',
        sig3_image: props.settings.spr_signatures?.sig3_image || null,
        sig4_title: props.settings.spr_signatures?.sig4_title || 'Penanggung Jawab',
        sig4_name: props.settings.spr_signatures?.sig4_name || '',
        sig4_image: props.settings.spr_signatures?.sig4_image || null,
    },
...
const addTerm = () => {
    form.spr_terms_conditions.push('Poin ketentuan baru...');
};

const removeTerm = (index) => {
    if (form.spr_terms_conditions.length > 1) {
        form.spr_terms_conditions.splice(index, 1);
    }
};
            ...
            <div class="lg:col-span-2">
                ...
                <div v-if="activeTab === 'spk'" class="space-y-6">
                    ...
                    <div class="bg-white rounded-3xl border border-slate-100 p-4 sm:p-8 shadow-sm space-y-6">
                        <h3 class="text-xs font-black uppercase tracking-widest text-blue-600 mb-6">1. Identitas Perusahaan & Header Dokumen SPR</h3>
                        <div class="space-y-4">
                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-1.5">Nama Perusahaan / Developer</label>
                                <input v-model="form.company_name" type="text" placeholder="misal: PT. SERANGKAI RODEN DEVELOPMENT" class="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-blue-600/20" />
                            </div>
                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-1.5">Alamat Kantor / Proyek</label>
                                <textarea v-model="form.company_address" rows="2" placeholder="misal: Cilandak Timur, Jakarta Selatan" class="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-blue-600/20"></textarea>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-[10px] font-black text-slate-400 uppercase mb-1.5">Website Official</label>
                    ...
                    <!-- Section 2: Logo Developer / Proyek -->
                    <div class="bg-white rounded-3xl border border-slate-100 p-4 sm:p-8 shadow-sm space-y-6">
                        <h3 class="text-xs font-black uppercase tracking-widest text-pink-600 mb-6">2. Logo Resmi Dokumen SPR</h3>
                        <div class="flex flex-col sm:flex-row items-center sm:space-x-8 gap-6 sm:gap-0">
                            <div class="aspect-video w-48 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden p-4 shrink-0">
                                <img v-if="settings.company_logo" :src="`/storage/${settings.company_logo}`" class="max-h-16 object-contain" />
                            ...
                            <div class="flex-1 text-center sm:text-left">
                                ...
                                <p class="text-[9px] text-slate-400 mt-2 italic leading-relaxed">Logo ini akan menjadi header utama pada cetakan Surat Pemesanan Rumah (SPR).</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Section 4: Catatan-Catatan (Terms & Conditions Dinamis List) -->
                    <div class="bg-white rounded-3xl border border-slate-100 p-4 sm:p-8 shadow-sm space-y-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-xs font-black uppercase tracking-widest text-emerald-600">4. Catatan-Catatan & Ketentuan SPR (Dinamis)</h3>
                                <p class="text-[10px] text-slate-400 mt-0.5">Daftar pasal dan syarat ketentuan yang akan muncul di dokumen SPR.</p>
                            </div>
                            <button type="button" @click="addTerm()" class="px-4 py-2 bg-emerald-50 text-emerald-600 font-bold rounded-xl text-xs hover:bg-emerald-100 transition-all flex items-center gap-1.5">
                                <span>➕</span> <span>Tambah Poin Catatan</span>
                            </button>
                        </div>

                        <div class="space-y-3">
                            <div v-for="(term, idx) in form.spr_terms_conditions" :key="idx" class="flex items-start gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200">
                                <span class="w-6 h-6 rounded-lg bg-emerald-100 text-emerald-700 font-black text-xs flex items-center justify-center shrink-0 mt-1">
                                    {{ idx + 1 }}
                                </span>
                                <textarea v-model="form.spr_terms_conditions[idx]" rows="2" class="flex-1 bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-emerald-500/20 text-slate-800 leading-relaxed"></textarea>
                    ...
                    <div class="pt-4 flex justify-end">
                        <button @click="submit" class="px-10 py-4 bg-slate-900 text-white font-black rounded-2xl shadow-xl hover:-translate-y-1 transition-all uppercase text-xs tracking-widest flex items-center gap-2">
                            <span>💾</span>
                            <span>SIMPAN SELURUH TEMPLATE SPR & TTD</span>
                        </button>
                    </div>
                </div>
